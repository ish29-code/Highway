          ...
          <div className="flex flex-wrap gap-2 mt-3 ">
            {item.dates.map((d: string) => (
              <button
                key={d}
                onClick={() => setDate(d)}
                className={`px-3 py-1.5 rounded bg-gray-100 ${date === d ? 'bg-[#FACC15] text-gray-900 font-medium'
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {item.slots.map((s: any) => (
              <button
                ...
                className={`px-3 py-1.5 rounded bg-gray-100 disabled:opacity-50 ${time === s.time ? 'bg-[#FACC15] text-gray-900 font-medium'
      : 'bg-gray-100 hover:bg-gray-200 text-gray-700'}`}
              >
                {s.time} {s.left === 0
